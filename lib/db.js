...
export const addBoard = async (data) => {
  const ecr = cryptr.encrypt(JSON.stringify(data));
  const colRef = await db.doc(`users/${data.id}`).collection("boards");
  colRef.add({ ecr });
};
...
export const getTasks = async (uid, boardName) => {
  ...
  colRef.forEach(async (el) => {
    ...
    subtasks.forEach((subtask) => {
      let dcrSubtask = JSON.parse(cryptr.decrypt(subtask.data().ecrSubtask));
      subtasksArr.push({ ...dcrSubtask, id: subtask.id });
    });
    tasks.push({ ...dcr, subtasks: subtasksArr, id: el.id });
  });
  console.log(tasks);
  return tasks;
};
...
export const addColumn = async (data) => {
  // const ecrColumn = cryptr.encrypt(JSON.stringify(data.column));
  return await db
    .doc(`users/${data.uid}`)
    .collection("boards")
    .doc(`${data.boardId}`)
