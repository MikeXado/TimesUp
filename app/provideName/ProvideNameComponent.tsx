...
interface ProvideNameFormData {
  firstName: string;
  lastName: string;
}

export default function ProvideNameComponent({ uid }): JSX.Element {
  const router = useRouter();
  const [isLoading, setIsLoading] = useState<boolean>(false);
  const { register, handleSubmit } = useForm();
  const provideName = async (values: ProvideNameFormData): Promise<void> => {
    try {
      setIsLoading(true);
      const result = await fetch(`/api/v1/${uid}/user-profile/username`, {
        method: "POST",
        headers: {
  ...
  const onSubmit: SubmitHandler<ProvideNameFormData> = (data) => {
    provideName(data);
  };
